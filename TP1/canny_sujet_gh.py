...
def sample_image(I, x, y):
    ...
    return I[
        y,
        x]  # see http://docs.scipy.org/doc/numpy/user/basics.indexing.html#indexing-multi-dimensional-arrays
...
def smooth_gaussian(im, sigma):
    ...
    # vous pouvez utiliser np.meshgrid (utilisée dans la fonction display_image_and_vector_field ci dessus) avec la fonction np.arange
    ...
    halfwid = 3 * sigma
    X, Y = np.meshgrid(np.arange(-halfwid, halfwid), np.arange(-halfwid, halfwid))
    gaussian = np.exp(-1 / (2 * sigma**2) * (X**2 + Y**2))
    plt.imshow(gaussian)
    plt.title("Gaussian function")
    plt.show()

    # TODO make gaussian filter same size of the image ?
    print(im.shape)
    print(gaussian.shape)
    im_smooth = scipy.ndimage.convolve(im.astype(float), gaussian)
...
def gradient_norm_and_angle(gradient_x, gradient_y):
    # TODO:
...
def approx_angle_and_direction(angle):
    # TODO:
...
def local_maximum(norm_gradient, direction_x, direction_y):
    ...
    X, Y = np.meshgrid(np.arange(0, norm_gradient.shape[1]),
                       np.arange(0, norm_gradient.shape[0]))
...
def canny(im, sigma, threshold1, threshold2, display=True):
    ...
    #norm_gradient,angle=gradient_norm_and_angle(gradient_x,gradient_y)

    #angle_rounded,direction_x,direction_y=approx_angle_and_direction(angle)
    ...
    if display:
        ...
        pause()
        display_image_and_vector_field(im_smooth, gradient_x, gradient_y, 10,
                                       'b')
        pause()
        ...
        pause()
        display_image_and_vector_field(im_smooth, direction_x, direction_y, 15,
                                       'b')
        pause()
