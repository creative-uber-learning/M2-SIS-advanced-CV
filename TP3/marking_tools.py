...
class marking():
    ...
    def add(self, coef, student_solution, exact_solution, duration=None, max_duration=None, relative_coef_duration=0.5, tol=1e-8, nbdiff_max=0, isimage=False):
        if student_solution is None:
            ...
            try:
                ...
                if isinstance(student_solution, np.ndarray):
                    student_solution = student_solution.astype(
                        exact_solution.dtype)
                diff = np.abs(student_solution - exact_solution)
                max_error = np.max(diff)
                nbdiff = np.sum(diff > tol)
                valid = (nbdiff <= nbdiff_max)
                if nbdiff > 0 and valid:
                    print("WARNING : found " + str(nbdiff) +
                          " differences , but still accepted")
                if not(valid):
                    print('found ' + str(nbdiff) +
                          ' values with difference with exact solution is  greater that '+str(tol))
                    print('the greatest error is '+str(max_error))
                    if nbdiff_max > 0:
                        print('expect less than ' +
                              str(nbdiff_max)+' differences')

                    cov_mat = np.cov(
                        [exact_solution.flatten(), student_solution.flatten()])
                    if np.min(np.linalg.eig(cov_mat)[0]) < 1e-4:
                        print(
                            "seem like there is a affine relationship between your data and the expected ones")
                        plt.plot(exact_solution.flatten(),
                                 student_solution.flatten())
        ...
        print('question '+str(len(self.marks)) +
              ": "+str(valid*coef)+"/"+str(coef))
